...
(function(){
	...
	Loader.LoadLibrary = function(filename, typename, onprogress, finish)
	{
		...
		var func_loadNextNodes = function(){
			...
			// @fixme 暂时将并发数量钉死为1，通过测试发现貌似大于1的话，崩溃的概率就会增大
			// 测试帖子参照spp:ticket:1648#comment:68
			var MaxThread = 1//Global.MAX_THREAD;
		...
		if(rootnode)
		{
			...
			var nodeCount = typeNodeItr.GetEndPosition();
		...
		{
			System.Report("Failed to open file " + filename + "\n");
			if(typename != "sound")
			{
				// 缺少必须加载的就直接退出.
				exit(1);
	...
	Loader.LoadSector = function(filename, typename, processWorldNode, finish)
	{
		...
		// 在<world>中，除了<sector>和<sequences>节点，其他都删除。
		{
			while(worldChildItr.HasNext())
			{
				// 获取`<library>`节点
				var node = worldChildItr.Next();
				if(node.value == "sector" || node.value == "sequences")
					continue;
