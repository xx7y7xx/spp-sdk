...
// ==========================================================================================================
		...
		Event.Subscribe(function(e){
			...
			if(VideoWallData[player.clickedMesh] == undefined)
			{
				console.debug("该mesh以前没有定义过视频墙");
				// 创建临时的视频墙，在“定位”之后再保存到JSON中。
				player.tmpData = "";
			}
			else
			{
				console.debug("该mesh以前定义过视频墙了，读取数据");
				player.tmpData = VideoWallData[player.clickedMesh];
			}
			input_box.SetProperty("Text", player.tmpData);
			
			// 显示信息窗口。
			FUNCTION_DATA.get_windows("kuaisudingwei/name").SetProperty("Visible", "True");
			
		}, "tool.videowall.mesh.mouse.click");
