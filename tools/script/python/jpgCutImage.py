...
我们将按照您输入的数值进行裁剪:') + '\n'
			...
			newImg.save(targetdir + os.sep + fName + fpost)
			try:
...
for parent,dirnames,filenames in os.walk(targetdir):
	for filename in filenames:
		filename = targetdir + os.sep + filename
		myimg = Image.open(filename)
		print str(filename) + ' mode is: ' + str(myimg.mode) + '\n'

img = Image.open(r'c:\users\mingl_wang\desktop\test1\b.jpg')
print img.mode