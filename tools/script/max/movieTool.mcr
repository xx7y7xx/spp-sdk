macroScript MoveTool
category:"Superpolo"
ButtonText:"Scene MovieTool" 
tooltip:"MovieTool" Icon:#("Maxscript",7)
(
	/**
	 * @brief 得到项目的路径 
	...
	(
		...
		(
			...
			(
				...
				(
					if(classof o.material[m] == Multimaterial) then
					(
						--messagebox "请联系作者"
						print "子材质里有多材质，没有处理"
					)
					else
					--if classof o.material [m] != Standardmaterial then
					(
						tt = Standardmaterial ()
						testflag = false
						try(
		...
		-- 			print arr_id
			...
			*/
			...
			(	
				...
				(
					select selectobj
					if classof selectobj != Editable_Poly then converttopoly obj
					subobjectLevel = 5
				)
				messagebox "材质整理完毕"
			)
