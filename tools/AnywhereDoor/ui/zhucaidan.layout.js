	...
	ZHUCAIDAN_LAYOUT = {
		...
		method : {
			...
			school_quick_position_export : function(){
				...
				var jsonString = "AnywhereDoorData = " + JSON.stringify(AnywhereDoorData, null, "    ");
			...
			// 输入tip。
			"kuaisudingwei_tishi_named" :function(){
				player.isClickOnUI = true;
				...
				var tip = input_box.GetProperty("Text");
				if(tip == "")
				{
					alert("请输入任意门的提示文字！");
					return;
				...
				player.tmpData.tip = tip;
				
				alert("提示文字输入成功！下面请为任意门定位");
			},
			// 为任意门定位
			"kuaisudingwei_tishi_ok" :function(){
				...
				// 必须先命名再定位。
				if(player.tmpData.tip == "")
				{
					alert("请先为任意门输入提示文字，然后再进行定位！")
					return;
				}
				...
				if(isNaN(pos.x) || isNaN(pos.y) || isNaN(pos.z) || isNaN(rot.y)){
					...
					player.tmpData.pos = {
						"reach_position_x": pos.x, 
						"reach_position_y": pos.y, 
						"reach_position_z": pos.z, 
						"reach_rotation_y": rot.y
					};
				}
				
				AnywhereDoorData[player.clickedMesh] = player.tmpData;
				
				alert("创建任意门成功 : " + player.clickedMesh + "\n",
					"请指定下一个任意门！"
				);
			},
		...
		window : {
			...
			"kuaisudingwei/chinses_name":{
				property:{
					associatedWindow : function (obj, propt_name){
						obj[propt_name]=['kuaisudingwei/name','kuaisudingwei/chinses_name','kuaisudingwei/ok','kuaisudingwei/named'];
					}
				},
				event : {
					"Activated":"serch_ecitbox_Activated",
			...
			"kuaisudingwei/named":{
				...
				event : {
					"Clicked":"kuaisudingwei_tishi_named"
				},
				subscribe : {}
			},
