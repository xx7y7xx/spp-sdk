...
require("importer/importer.js");
	...
	var callbacks = {
		Choice:{"effectTree":false,"mergexmls":true},//控制是否加载特殊材质处理，以及选择哪处理
			...
			for(it = 0;it < material.GetTextureCount();it++)       //遍历aimaterial所有的texture节点
			{
				...
				else      //diffuse贴图存在
				{
					...
					else if(ifeffect == 'Y')    //有效果
					{
						...
						var nmapname = textRealName.replace(".","_n.");
				
						if(VFS.Exists(nmapname))
						{
						...
						var smapname = textRealName.replace(".","_s.");
						
						
						if(VFS.Exists(smapname))
						{
							importer.addTextureNode(textures,
                                smapname);
						}
						
						var hmapname = textRealName.replace(".","_h.");
						
						if(VFS.Exists(hmapname))
						{
							importer.addTextureNode(textures,
                                hmapname);
						}
		...
		onMaterialAdd:function(materials,material,meshname)   //materials为iDocumentNode对象，material为aiMaterial对象
		{
			...
			if(isCheckmat)
			{
				matname = importer.CheckMaterial(materials,material);
				if(matname != false)
				{
					return matname;
				}
			...
			matname = material.GetPropertyName(0);
			if(ifeffect == 'N')      //如果没有效果
			{
					...
					else    // ifmaterialDef == false 或没有赋值    这部分会执行
					{
						...
						for(it = 0;it < material.GetTextureCount();it++)   //扫描aimaterial下所有的texture
						{
							...
							if(VFS.Exists(textRealName))   //贴图存在,开始创建相关节点
							{
								...
								if(expandedname.toLowerCase() == "png")
								{
								var diffusenode = materialnode.CreateChild(xmlDocument.NODE_ELEMENT);
								diffusenode.value="shader";
								diffusenode.SetAttribute("type","diffuse");
								var diffusevalue = diffusenode.CreateChild(xmlDocument.NODE_TEXT);
								diffusevalue.value = "*null";
...
/* 	AssertTrue(load("/importer/mergexmls.js"),
	...
	if(!load("/meshgen_names.js")){
		 System.exitcode = 45;		
		 // alert("Error Code 45 : Can not load 'meshgen_names.js',请重新使用scenetools工具构建!");
		 exit();				
	}
	//获取输入路径下所有文件名称
	...
	if(convertmode == "factory") 
	{
		...
		matproxml = new xmlDocument();
		if(VFS.Exists("/matpro.xml"))
		{
	
			var profile = VFS.Open("/matpro.xml");
			matproxml.Parse(profile);
			matprort = matproxml.root.GetChild("materials");
			if(matprort == null)
			{
				alert("can not parse the file /matpro.xml");
				exit();
			}
			// alert(matprort.GetChild("m_60_m_62sub1"));
			// exit();
		}
		else   //outputpath/facpro.xml不存在，创建一个模板
		{
			alert("can not found the file matpro.xml");
			exit();
		}
		...
		{
			var tworldfile = VFS.Open("/tools/world.xml");
	...
	AssertTrue(load("/tools/utils.js"),
		"failed to load `utils.js` ",
		"OR script run error.");
	...
	if(convertmode == "factory")
	{
		matfile = VFS.Open("outputpath/materials.xml", VFS.WRITE);
		if(callbacks.Choice["mergexmls"])
		{
			if(mg.isopenxml == true)
			{	
				var wtskylabel = mg.getskywater();
				if(mg.wtskyflag)
				{
					var liblabel = matxml.root.GetChild("library");
					var matslabel = liblabel.GetChild("materials");
					var matname = matslabel.CreateChild(xmlDocument.NODE_ELEMENT);
					matname.value = "material";
					matname.SetAttribute("name", "sky_001");
					var matlab = wtskylabel.GetChild("material");
					CopyNodes(matname,matlab);
				}
			}
		}
		matxml.WritetoFile(matfile);
		
		profile = VFS.Open("outputpath/facpro.xml", VFS.WRITE);
		facproxml.WritetoFile(profile);
		//将修改后的xml文件写到目标路径下的factorylib.xml文件中	
		faclibfile = VFS.Open("outputpath/factorylib.xml", VFS.WRITE);
		if(callbacks.Choice["mergexmls"])
		{
			if(mg.isopenxml == true)
			{
				if(mg.wtskyflag)
				{
					//add sky water to faclib.xml
					var liblabel = factorylib.root.GetChild("library");
					var library = liblabel.CreateChild(xmlDocument.NODE_ELEMENT);
					library.value = "library";
					var libc = library.CreateChild(xmlDocument.NODE_TEXT);
					libc.value = "/art/factories/skydome_frankieisland_SkyDome.xml";
					VFS.Copy("/data/effectextures/skydome_frankieisland_SkyDome.xml", "outputpath/factories/skydome_frankieisland_SkyDome.xml");
					//add meshobj "skydome_frankieisland_SkyDome" to world.xml
					var MeshObjs = tworld.GetChild("sector");
					var wtskyobj = MeshObjs.CreateChild(xmlDocument.NODE_ELEMENT);
					wtskyobj.value = "meshobj";
					wtskyobj.SetAttribute("name", "skydome_frankieisland_SkyDome");
					var meshlab = wtskylabel.GetChild("meshobj");
					CopyNodes(wtskyobj,meshlab);
				}
			}
		}
		factorylib.WritetoFile(faclibfile);
		//在导出world.xml时,根据meshgen模型分别导出不同xml里
		if(callbacks.Choice["mergexmls"])
		{
			if(lod.isopenxml == true)
			{
				VFS.Copy("/water_lod.js","outputpath/water_lod.js");
			}
		}
		...
		if(callbacks.Choice["mergexmls"])
		{
			var meshgentoggle = true;
			//Toggle meshgen
			if(meshgentoggle == true){
				
				/* 	if(!load("/meshgen_names.js")){
						alert("load 'meshgen_names.js' failed,plz rebuild!")
					} */
				//将meshgen_names.js文件copy到art目录下,供meshgen使用
				VFS.Copy("/meshgen_names.js","outputpath/meshgen_names.js");
			}
		}
		
		//将修改后的xml文件写到目标路径下的world.xml文件中
		wfile = VFS.Open("outputpath/world.xml", VFS.WRITE);
